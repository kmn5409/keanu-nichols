...
Goal 3:<br>
This is a solution to <a href="http://127.0.0.1:4000/gsoc/2018/07/27/gsoc-Week-11.html#here">last week’s</a> problem whereby the Github rate limit would be exceeded so the program would stop. This goes about uploading 500 lines in the Database and if the limit is exceeded the program sleeps. The problem is that it sleeps for around 1 hour.
{% highlight python linenos %}
for y in repo_url:       
       repo = GitHub(owner=own,repository=y,api_token=token,\
